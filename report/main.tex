...
\begin{figure}[tb]
\centering
\includegraphics[width = 0.4\hsize]{imperial}
...
In accordance with \textit{The Greenhouse Gas Protocol Corporate Accounting and Reporting Standard}, many jurisdictions require (or will soon require) mandatory Scope 1 and Scope 2 disclosures for publicly listed firms, including in the EU \cite{eu20232772}, the US \cite{sec2024}, the UK \cite{ukleg2013} and Japan \cite{fsa2022}. Furthermore, although at the moment Scope 3 reporting is optional in most of these key financial markets, the regulatory landscape is shifting away from voluntary efforts and towards mandatory Scope 3 disclosures \cite{ftserussell2024}. 
\\ \\
...
\section{Estimating Scope 3 emissions}\label{sec:Scope3Modelling}

Notwithstanding the problems with Scope 3 data, many papers have either tried to estimate Scope 3 emissions, either to fill in the missing gaps or to forecast future emissions. 

\subsection{Modelling techniques}

There is no industry consensus on the best way to approach estimating GHG emissions \cite{FTSERussell2022}. Consequently, the standard tactic is to try a battery of methods and empirically observe which is the best for a particular exercise. 

\subsection{Filling in missing Scope 3 data}

While Scope 3 data may be missing, firms often report their Scope 1 and Scope 2 emissions. Unfortunately, with the exception of the utilities sector, a simple regression of Scope 1 and 2 emissions on Scope 3 reported emissions yields very little explanatory power \cite{ManGroup2022}. 

\subsection{Forecasting Scope 3 emissions}
