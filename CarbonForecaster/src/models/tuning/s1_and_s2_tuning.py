...
def generate_param_combinations():
    ...
    y_choice_list = [['s1_and_s2_co2e'], ['s1_and_s2_co2e_intensity']]
    x_choice_str_list = ['extended', 'core', 'limited']
    observation_filter_list = [None, 'covid', 'material']
    ...
    for y_choice, x_choice_str, observation_filter in itertools.product(y_choice_list, x_choice_str_list, observation_filter_list):
        ...
        # S1 and S2 doesn't use a materiality filter 
        n_folds = 10
...
# Or run it one at a time
min_firms_per_fold = 3
n_folds = 10
y_choice = ['s1_and_s2_co2e_intensity']
x_choice_str = 'core'
scaler_choice = None # log scaling used if scaler_choice is None. Scalar only specified for 'extended' x_choice_str
year_lower = 2016
year_upper = 2022
observation_filter = "material" # Valid values are None, "covid", and "material"
seed_choice = 42
np.random.seed(seed_choice)

# Data preparation
data_train_scaled, data_test_scaled, data_test, X_train_scaled, X_test_scaled, x_choice, x_scaled, y_train_scaled, y_test_scaled, y_test, custom_folds_for_training_data, scaler_str, observation_filter_str = \
    prepare_data_pipeline(data_file_path='data/ftse_world_allcap_clean.csv', y_choice=y_choice, x_choice_str=x_choice_str, 
                          scaler_choice=scaler_choice, year_lower=year_lower, year_upper=year_upper, observation_filter=observation_filter, 
                          min_firms_per_fold=min_firms_per_fold, n_folds=n_folds, seed_choice=seed_choice)

# Hyperparameter tuning
run_all_model_tuning(X_train=X_train_scaled, y_train=y_train_scaled, custom_folds=custom_folds_for_training_data, 
                     save_path='src/models/tuned', y_choice=y_choice, x_choice_str=x_choice_str, observation_filter_str=observation_filter_str, 
                     year_lower=year_lower, year_upper=year_upper, scaler_str=scaler_str, seed_choice=seed_choice)