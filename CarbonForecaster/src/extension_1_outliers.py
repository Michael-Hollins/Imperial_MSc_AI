...
if False:
    ...
    historical_cols = get_fundamentals_cols(raw_data)
    df = standardise_missing_values(raw_data)
    financial_years = ['FY' + str(x) for x in range(2016, 2024)]
    ...
    df = consolidate_observations(df, group_cols=grp_cols, historical_cols=historical_cols)
    df = drop_rows_with_all_missings(df, fields_to_check=historical_cols)
    df = drop_dual_listing_duplicates(df)
    df = remove_fundamentals_with_low_coverage(df, cols_to_check=historical_cols, threshold=0.1)
    df = replace_and_fill_zeros_in_fundamentals(df, covariates=get_fundamentals_cols(df))
    df = map_country_classification(df, country_classification=country_classification)
    firm_B_raw = df[df['instrument'] == firm_B]
    firm_B_raw = convert_emissions_to_intensities(firm_B_raw)
    firm_B_raw.to_csv('data/firm_B_raw.csv', index=False)
