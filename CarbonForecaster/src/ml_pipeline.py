...
# Old code
# def unscale_predictions(y_pred_scaled, X_scaled_data, x_choice_vars, scaler, y_choice, fundamentals, emission_scores, other_emission_categories):
#     if y_choice[0] in ['s3_upstream', 's3_downstream', 's3_cat_total']:
#         x_vars_scaled = fundamentals + emission_scores + other_emission_categories
#     else:
#         x_vars_scaled = fundamentals + emission_scores
#     X_scaled_transformed = pd.DataFrame(X_scaled_data, columns=x_choice_vars)[x_vars_scaled]
#     data_scaled = np.column_stack((X_scaled_transformed, y_pred_scaled))
#     y_pred_unscaled = pd.Series(scaler.inverse_transform(data_scaled)[:, -1])
#     return y_pred_unscaled

# def get_features(df, y_choice):
#     fundamentals = ['mcap', 'revenue', 'ebit', 'ebitda', 'net_cash_flow', 'assets', 'receivables', 'net_ppe', 'capex', 'intangible_assets', 'lt_debt']
#     emission_scores = ['policy_emissions_score', 'target_emissions_score']
#     other_emission_categories = ['s1_co2e', 's2_co2e']
#     financial_year_dummies = [x for x in df.columns if "financial_year_" in str(x)]
#     econ_type_dummies = [x for x in df.columns if 'cc_classification_' in str(x)]
#     peer_dummies = [x for x in df.columns if 'peer_group_' in str(x)]
    
#     if y_choice[0] in ['s3_upstream', 's3_downstream', 's3_cat_total']: # if we have a scope 3 target variable, include S1 and S3 as regressors
#         x_choice = fundamentals + emission_scores + other_emission_categories + financial_year_dummies + econ_type_dummies + peer_dummies
#     else:
#         x_choice = fundamentals + emission_scores + financial_year_dummies + econ_type_dummies + peer_dummies
    
#     return x_choice, fundamentals, emission_scores, other_emission_categories, financial_year_dummies, econ_type_dummies, peer_dummies


# def get_train_test_indices(data, y_choice, id_choice, x_choice, n_folds):
   
#     data_choice = data.copy()
#     df = data_choice[id_choice + y_choice + ['fold'] + x_choice].dropna().reset_index(drop=True)
    
#     # Pick a random fold for our test fold
#     random_test_fold = np.random.randint(1, n_folds)
#     print(f'\nThe random test fold is {random_test_fold}')
...
# def model_tune(model, param_grid, X_train, y_train, custom_folds, save_path, model_name, tune=True):
#     if tune:
#         search = GridSearchCV(model, param_grid, scoring='neg_mean_absolute_error', cv=custom_folds, n_jobs=-1, verbose=2)
#         search.fit(X_train, y_train.ravel())
...
#     tuned_model = pickle.load(open(f'{save_path}/{model_name}.pickle', 'rb'))
#     return tuned_model